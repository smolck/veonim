...
const unzip = (src, dest) =>
  new Promise((done) =>
    Archiver(['open', src, dest]).on('exit', done).on('error', done)
  )

module.exports = ({ org, repo, tag, mac, win, linux }, dlpath) =>
  new Promise(async (done, fail) => {
    ...
    if (alreadyExists)
      return console.log(
        `binary "${repo}" already exists in "${dlpath}" skipping download`
      )
    ...
    stream
      ...
      .on('close', async () => {
        ...
        const unzipErr = await unzip(filename, tempPath).catch((e) =>
          console.error('unzip err:', e)
        )
