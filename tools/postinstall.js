...
const vscodeTypings = () =>
  new Promise(async (done, fail) => {
    ...
    const vscodeTypingsUrl = (version) =>
      `https://raw.githubusercontent.com/Microsoft/vscode/${version}/src/vs/vscode.d.ts`

    await fs.ensureDir(fromRoot(modulePath))
    await fs.writeFile(
      fromRoot(modulePath, 'package.json'),
      `{
  "name": "@types/vscode",
  "main": "",
  ...
  "typings": "index.d.ts"
    ...
    downloadStream.pipe(fileStream).on('close', done).on('error', fail)
  })

require.main === module &&
  go(async () => {
    $`installing vscode extension api typings`
    await vscodeTypings().catch((err) =>
      console.log('failed to install vscode typings', err)
    )
