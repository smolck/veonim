...
const findPackageJson = (path: string): Promise<string> =>
  new Promise((done, fail) => {
    const results = [] as string[]
    const rg = Ripgrep(
      ['--files', '--glob', '!node_modules', '--glob', '**/package.json'],
      { cwd: path }
    )
    rg.stderr!.pipe(new NewlineSplitter()).on('data', fail)
    rg.stdout!.pipe(new NewlineSplitter()).on('data', (pkgpath: string) =>
...
const getExtensionConfig = async (
  packagePath: string
): Promise<ExtensionPackageConfig> => {
  if (!packagePath)
    throw new Error(`packagePath does not exist. oopsie doopsie`)
  const fileExists = await exists(packagePath)
  if (!fileExists)
    throw new Error(`extension package.json does not exist: ${packagePath}`)
...
const getExtensionConfigsFromFS = async () => {
  ...
  const missingPackageJsonPaths = extensionPaths.filter(
    (m) => !m.packageJsonPath
  )
  if (missingPackageJsonPaths.length) {
    const paths = missingPackageJsonPaths.map((m) => m.path)
    console.error(
      'these extension dirs have no package.json files. wut?',
      paths
    )
  }
...
const installMaybe = async (userDefinedExtensions?: string[]) => {
  ...
  const recursiveResolveExtensions = async (): Promise<
    ExtensionPackageConfig[]
  > => {
    const extensions = await getExtensionConfigsFromFS()
    const dependencies = extensions.reduce(
      (exts, e) => [...exts, ...(e.extensionDependencies || [])],
      [] as string[]
    )
    const extensionsNotInstalled = dependencies.filter(
      (dep) => !extensions.some((e) => e.id === dep)
    )
...
copy(copyFrom, copyTo, { overwrite: true }).catch((err) => {
  console.error(
    'failed to copy bundled extension dependencies',
    copyFrom,
    copyTo,
    err
  )
