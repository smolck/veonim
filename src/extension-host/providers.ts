...
// lol
type MassageProviderResult<S> = AlterReturnType<
  S,
  Promise<
    | Unpacked<
        Exclude<Exclude<ReturnTypeOf<S>, vsc.ProviderResult<S>>, Thenable<any>>
      >[]
    | undefined
  >
>
...
const F = <T extends object>() => {
  ...
  const register = (filetypes: string[], items: T) =>
    ms.addMultiple(filetypes, items)
  ...
  interface Api {
    register(
      filetypes: string[],
      items: T
    ): ReturnType<MapSetter<string, T>['addMultiple']>
  }
...
const coalesce = <T extends any[]>(
  results: T
): NonNullable<Unpacked<Unpacked<T>>>[] =>
...
const api = {
  ...
  provideCompletionItems: (
    ...
    promise: (async () => {
      ...
      const results = await providers.completionItem.provideCompletionItems(
        document,
        position,
        token,
        context
      ...
      const completions = cache.currentCompletionItems.map((item, index) =>
        simplify.completionItem(item, index)
      )
  ...
  resolveCompletionItem: (item: vsc.CompletionItem, tokenId?: string) => ({
    ...
    promise: (async () => {
      const { token } = makeCancelToken(tokenId!)
      const realItem = Reflect.get(
        cache.currentCompletionItems,
        (item as any).index
      )
      const results = await providers.completionItem.resolveCompletionItem!(
        realItem,
        token
      )
  ...
  getCompletionTriggerCharacters: () => ({
    ...
    promise: (async () => {
      return [
        ...(providers.completionTriggerCharacters.get(nvim.state.filetype) ||
          []),
      ]
    })(),
  }),
  provideCodeActions: (context: vsc.CodeActionContext, tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const results = await providers.codeAction.provideCodeActions(
        document,
        range,
        context,
        token
  ...
  provideDefinition: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.definition.provideDefinition(
        document,
        position,
        token
      )
      ...
      return {
        path: (
          (location as vsc.Location).uri ||
          (location as vsc.LocationLink).targetUri
        ).path,
        range: threadSafeObject(
          (location as vsc.Location).range ||
            (location as vsc.LocationLink).targetRange
        ),
      }
    })(),
  }),
  provideImplementation: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.implementation.provideImplementation(
        document,
        position,
        token
      )
  ...
  provideTypeDefinition: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.typeDefinition.provideTypeDefinition(
        document,
        position,
        token
      )
  ...
  provideDeclaration: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.declaration.provideDeclaration(
        document,
        position,
        token
      )
  ...
  provideHover: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.hover.provideHover(
        document,
        position,
        token
      )
      ...
      return hover.contents
        .reduce((res, markedString) => {
          const text =
            typeof markedString === 'string' ? markedString : markedString.value
          return [...res, text]
        }, [] as string[])
        .filter((m) => m)
    })(),
  ...
  provideDocumentSymbols: (tokenId?: string) => ({
    ...
    promise: (async () => {
      const { token } = makeCancelToken(tokenId!)
      const document = TextDocument(nvim.current.buffer.id)
      const results = await providers.documentSymbol.provideDocumentSymbols(
        document,
        token
      )
      if (!results) return
      ...
      const res = dedupOn(
        symbols,
        (a, b) => a.name === b.name && rangesEqual(a.range, b.range)
      )
  ...
  provideWorkspaceSymbols: (query: string, tokenId?: string) => ({
    cancel,
    promise: (async () => {
      const providerSet: MapSetter<
        string,
        vsc.WorkspaceSymbolProvider
      > = Reflect.get(providers.workspaceSymbol, $$GET_PROVIDERS)
      ...
      const results = await Promise.all(
        [...providerz].map((p) => p.provideWorkspaceSymbols(query, token))
      ).then(coalesce)
  ...
  resolveWorkspaceSymbol: (
    ...
    promise: (async () => {
      ...
      const results = await Promise.all(
        [...providerz].map((p) => {
          return (
            p.resolveWorkspaceSymbol && p.resolveWorkspaceSymbol(symbol, token)
          )
        })
      ).then(coalesce)
  ...
  provideReferences: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const results = await providers.reference.provideReferences(
        document,
        position,
        context,
        token
  ...
  prepareRename: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.rename.prepareRename!(
        document,
        position,
        token
      )
  ...
  renameSymbol: (position: Position, newName: string, tokenId?: string) => ({
    cancel,
    promise: (async () => {
      const providerSet: MapSetter<string, vsc.RenameProvider> = Reflect.get(
        providers.rename,
        $$GET_PROVIDERS
      )
      ...
      const result = await Promise.race([
        Promise.all(
          [...providerz].map((p) =>
            p.provideRenameEdits(document, position, newName, token)
          )
        ).then(coalesce),
      ...
      const editRequests = (result as vsc.WorkspaceEdit[]).map(
        workspace.applyEdit
      )
  ...
  provideDocumentFormattingEdits: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const edits = await providers.documentFormattingEdit.provideDocumentFormattingEdits(
        document,
        options,
        token
      )
  ...
  provideDocumentRangeFormattingEdits: (
    ...
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const options = await getFormattingOptions()
      const edits = await providers.documentRangeFormattingEdit.provideDocumentRangeFormattingEdits(
        document,
        range,
        options,
        token
  ...
  provideOnTypeFormattingEdits: (character: string, tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const position = new Position(nvim.state.line, nvim.state.column)
      const options = await getFormattingOptions()
      const edits = await providers.onTypeFormattingEdit.provideOnTypeFormattingEdits(
        document,
        position,
        character,
        options,
  ...
  provideSignatureHelp: (
    context: vsc.SignatureHelpContext,
    tokenId?: string
  ) => ({
    cancel,
    promise: (async () => {
      ...
      const document = TextDocument(nvim.current.buffer.id)
      const position = new Position(nvim.state.line, nvim.state.column)
      const results = await providers.signatureHelp.provideSignatureHelp(
        document,
        position,
        token,
        context
  ...
  provideDocumentLinks: (tokenId?: string) => ({
    ...
    promise: (async () => {
      const { token } = makeCancelToken(tokenId!)
      const document = TextDocument(nvim.current.buffer.id)
      const results = await providers.documentLink.provideDocumentLinks(
        document,
        token
      )
      if (!results) return
  ...
  resolveDocumentLink: (link: vsc.DocumentLink, tokenId?: string) => ({
    ...
    promise: (async () => {
      const { token } = makeCancelToken(tokenId!)
      const results = await providers.documentLink.resolveDocumentLink!(
        link,
        token
      )
      return results && results[0]
  ...
  provideColorPresentations: (
    ...
    promise: (async () => {
      ...
      const colors = await providers.color.provideColorPresentations(
        color,
        context,
        token
      )
  ...
  provideDocumentColors: (tokenId?: string) => ({
    ...
    promise: (async () => {
      const { token } = makeCancelToken(tokenId!)
      const document = TextDocument(nvim.current.buffer.id)
      const colors = await providers.color.provideDocumentColors(
        document,
        token
      )
      if (!colors) return
  ...
  provideFoldingRanges: (tokenId?: string) => ({
    ...
    promise: (async () => {
      ...
      const ranges = await providers.foldingRange.provideFoldingRanges(
        document,
        options,
        token
      )
  ...
  getSignatureHelpTriggerCharacters: () => ({
    ...
    promise: (async () => {
      return [
        ...(providers.signatureHelpTriggerCharacters.get(nvim.state.filetype) ||
          []),
      ]
    })(),
  }),
  getOnTypeFormattingTriggerCharacters: () => ({
    ...
    promise: (async () => {
      return [
        ...(providers.onTypeFormattingTriggerCharacters.get(
          nvim.state.filetype
        ) || []),
      ]
    })(),
