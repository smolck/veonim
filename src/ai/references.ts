...
const groupResults = (m: LocationResult[]): ReferenceResult[] => [
  ...m.reduce((map, ref: LocationResult) => {
    map.has(ref.path) ? map.get(ref.path)!.push(ref) : map.set(ref.path, [ref])
...
nvim.onAction('references', async () => {
  const results = await boss.schedule(vscode.language.provideReferences(), {
    timeout: 3e3,
  })
  if (!results) return
  ...
  const keyword = ref1.lineContents.slice(
    ref1.range.start.character,
    ref1.range.end.character
  )
...
nvim.onAction('next-usage', async () => {
  const references = await boss.schedule(vscode.language.provideReferences(), {
    timeout: 2e3,
  })
  if (!references) return

  const { line, column, absoluteFilepath } = nvim.state
  const reference = findNext<Reference>(
    references,
    absoluteFilepath,
    line,
    column
...
nvim.onAction('prev-usage', async () => {
  ...
  const { line, column, absoluteFilepath } = nvim.state
  const reference = findPrevious<Reference>(
    references,
    absoluteFilepath,
    line,
    column
