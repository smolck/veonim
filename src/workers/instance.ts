...
on.nvimHighlightSearchPattern(async (pattern: string, id?: number) => nvim.highlightSearchPattern(pattern, id))
on.nvimRemoveHighlightSearch(async (id: number, pattern?: string) => nvim.removeHighlightSearch(id, pattern))
