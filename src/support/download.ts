...
export const url = {
  ...
  veonim: (repo: string) =>
    `https://github.com/veonim/${repo}/archive/master.zip`,
  vscode: (publisher: string, name: string, version = 'latest') =>
...
const downloadZip = (url: string, path: string) =>
  new Promise(async (done) => {
    const downloadStream = await fetchStream(url)
    const fileStream = createWriteStream(`${path}.zip`)

    downloadStream.pipe(fileStream).on('close', done).on('error', done)
  })

const unzip = (path: string) =>
  new Promise((done) =>
    Archiver(['unarchive', `${path}.zip`, path])
      .on('close', done)
      .on('error', done)
  )
