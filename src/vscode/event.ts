// https://raw.githubusercontent.com/Microsoft/vscode/master/src/vs/base/common/event.ts
/*---------------------------------------------------------------------------------------------
...
export namespace Event {
  ...
  export function echo<T>(
    ...
    const flush = (listener: (e: T) => any, thisArgs?: any) =>
...
export class AsyncEmitter<T extends IWaitUntil> extends Emitter<T> {
  ...
  async fireAsync(
    eventFn: (thenables: Promise<any>[], listener: Function) => T
  ): Promise<void> {
    if (!this._listeners) {
      return
    }
